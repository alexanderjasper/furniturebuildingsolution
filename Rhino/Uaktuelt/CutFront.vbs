...
sub main
	...
	If doc.Layers("Sockets") IsNot Nothing Then
		...
		db.SetValue(plateThickness - 11.0)
	...
	If doc.Layers("Plates") IsNot Nothing Then
		...
		db.SetValue(plateThickness - firstPassDepth)
		profile.DepthIncrement = db
	
		db.SetValue(firstPassDepth)
		...
		dim profile2 = new CamBam.CAM.MOPProfile(doc,doc.Layers("Plates").Entities.ToArray())
	
		db.SetValue(10.0)
		profile2.ToolDiameter = db

		it.SetValue(2)
		profile2.ToolNumber = it
	
		db.SetValue(2000.0)
		profile2.PlungeFeedrate = db
	
		db.SetValue(8000.0)
		profile2.CutFeedrate = db
	
		it.SetValue(18000)
		profile2.SpindleSpeed = it
	
		db.SetValue(plateThickness - depth)
		profile2.DepthIncrement = db
	
		db.SetValue(depth)
		profile2.TargetDepth = db

		db.SetValue(plateThickness + 15.0)
		profile2.ClearancePlane = db

		db.SetValue(plateThickness)
		profile2.StockSurface = db

		CamBamUI.MainUI.InsertMOP(profile2)
